...
class Level {
  ...
  for (const room of this.features.filter(
    (f) => ['room', 'corridor'].includes(f.get('indoor'))
  )) {
  ...
  const levelArea = this.features.find(
    (f) =>  f.get('indoor') === 'level'
  );
  if (levelArea) {
...
export class BuildingTopologySource extends VectorSource {
  ...
  private getLevels = (level: number, geo: jsts.geom.Geometry): Level[] => {
    const intersecting = this.levels.filter(
      (l) => l.levelNumber === level && l.geometry.intersects(geo)
    );
